...
- 하지만 메모리는 매우 많은 비트로 구성되어 있기 때문에 비트 단위로 위치를 확인하는 것은 비효율적이다.
...
- 사용자가 값을 넣었으리라고 예상했지만, 그렇지 않은 경우에 자바스크립트 엔진이 할당해주는 값

1. 값을 대입하지 않은 변수: 식별자가 데이터 영역의 메모리 주소를 지정하지 않은 경우
...
2. 존재하지 않은 프로퍼티: 없는 **메소드** 호출시 오류 발생
